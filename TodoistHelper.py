...
class TodoistHelper:
    ...
    def add_new_task(self, c_a, t_proj_id):
        """
        ...
        task_title = TodoistHelper.make_task_title(c_a)
        c_d = c_a['due_at']
        c_p = c_a['priority']
    ...
    def create_todoist_projects(self, course_names: list):
        """
        ...
        for i, course_name in enumerate(course_names):
            if not self.create_todoist_project(course_name):
                logging.info(
                    f"  {i + 1}. INFO: \"{course_name}\" already exists; skipping...")

    def create_todoist_project(self, course_name):
        if course_name in self.api.state['project_names']:
            logging.info(f" - INFO: Project already exists: \"{course_name}\"")
            return False

        self.api.projects.add(course_name)
